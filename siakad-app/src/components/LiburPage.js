...
export default function LiburPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Kelola Hari Libur</h2>
        <p className="text-slate-600 dark:text-white/40 text-sm mt-1">Tambah atau hapus hari libur khusus</p>
      </div>
      ...
      <div className="bg-white/50 dark:bg-white/5 border border-slate-300 dark:border-white/5 rounded-2xl p-6">
        ...
        {message && (
          <div className={`mt-3 px-4 py-2 rounded-xl text-sm ${
            message.type === 'success' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-300'
          }`}>{message.text}</div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-slate-600 dark:text-white/30 text-sm">Belum ada hari libur yang ditambahkan</div>
      ) : (
        <div className="space-y-2">
          {libur.map(l => (
            <div key={l.id} className="bg-white/50 dark:bg-white/5 border border-slate-300 dark:border-white/5 rounded-xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <span className="text-slate-600 dark:text-white/60 font-mono text-sm">{l.tanggal}</span>
                <span className="text-slate-900 dark:text-white font-medium text-sm">{l.keterangan}</span>
              </div>
              <button onClick={() => handleDelete(l.id)}
                className="px-3 py-1.5 text-red-400/60 hover:text-red-400 hover:bg-red-50 dark:bg-red-500/10 rounded-lg text-xs transition-all">
                Hapus
              </button>
            </div>
          ))}
