  ...
  return (
    <div className="space-y-6">
      ...
      {!isAdmin && (
        ...
        <span className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/10 text-white/60">
          Total: {data.length}
        </span>
      </div>
      )}
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/5 print:visible">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white/5">
                <th className="px-4 py-3 text-left text-white/50 font-medium">Tanggal</th>
                {isAdmin && <th className="px-4 py-3 text-left text-white/50 font-medium">Nama</th>}
            ...
            <tbody className="divide-y divide-white/5">
              {data.map((row, idx) => (
                <tr key={idx} className="hover:bg-white/3 transition-colors">
                  <td className="px-4 py-3 text-white/70 font-mono text-xs">{row.tanggal}</td>
                  {isAdmin && <td className="px-4 py-3 text-white font-medium">{row.nama_guru}</td>}
                  <td className="px-4 py-3 text-white/60 text-xs">{row.waktu}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-md text-xs ${
                      row.metode === 'NFC' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-blue-500/10 text-blue-400'
                    }`}>{row.metode || 'GPS'}</span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-white/40 text-xs">{row.catatan || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          ...
          .text-white\\/50, .text-white\\/70 { color: #333 !important; }
        }
      `}} />
    </div>
  );
